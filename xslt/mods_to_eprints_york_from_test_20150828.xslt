...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="v3:mods">
		<eprints>
			<eprint>
				...
				<!-- Document visibility -->
				<xsl:choose>
					<xsl:when test="v3:note[@type='researchoutput workflow state' and text()='approved']">
				...
				<pure_status><xsl:value-of select="v3:note[@type='researchoutput workflow state']" /></pure_status>
				<xsl:choose>
							...
							<item>
								<xsl:choose>
									<xsl:when test="v3:role/v3:roleTerm[@authority='pure/orcid']">
										<id><xsl:value-of select="v3:role/v3:roleTerm[@authority='pure/orcid']"/></id>
									</xsl:when>
									<xsl:otherwise>
				...
				<xsl:for-each select="v3:name[@type='personal' and v3:role/v3:roleTerm[@authority='pure/role'] != 'editor' and not(v3:namePart[@type = 'family'])]">
					<corp_creators><xsl:value-of select="v3:namePart[@type='given']"/></corp_creators>
				</xsl:for-each>
	...
	<xsl:template match="v3:relatedItem[@type='host']/v3:part/v3:detail[@type='volume']/v3:number">
		<series_volume><xsl:value-of select="." /></series_volume>
	</xsl:template>
	...
	<xsl:template match="v3:relatedItem[@type='host']/v3:part/v3:detail[@type='issue']/v3:number">
		<xsl:choose>
			...
			<xsl:otherwise>
				<series_number><xsl:value-of select="." /></series_number>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="bookTitleMatch">
		<xsl:if test="../v3:relatedItem[@type='host']/v3:titleInfo/v3:title">
			<book_title><xsl:value-of select="../v3:relatedItem[@type='host']/v3:titleInfo/v3:title" /></book_title>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="patentType">
		...
		<xsl:choose>
			<xsl:when test="../v3:subject[@xlin:type='ipc']/v3:topic">
				<id_number><xsl:value-of select="../v3:subject[@xlin:type='ipc']/v3:topic" /></id_number>
			</xsl:when>
			<xsl:when test="../v3:identifier[@type='patent_number']">
				<id_number><xsl:value-of select="../v3:identifier[@type='patent_number']" /></id_number>
			</xsl:when>
		</xsl:choose>
		<xsl:call-template name="journalMatch"/>
	</xsl:template>
